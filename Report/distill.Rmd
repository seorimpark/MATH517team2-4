...
```{r distWeight,fig.cap="Distribution of takeoff max weight (in Lbs)", echo=FALSE}
data_weight_no_zero=data
data_weight_no_zero=data_weight_no_zero[data_weight_no_zero$takeoff_max_weight..Lbs. != 0,]
hist(data_weight_no_zero$takeoff_max_weight..Lbs., main="Distribution of takeoff max weight (in Lbs)", xlab='Lbs', breaks =50)
```
```{r,include=FALSE}
...
```{r states, fig.cap="Number of accidents from 2006 to 2015 by state in Brazil", echo=FALSE, out.width="100%"}
...
```{r location, fig.cap="Number of accidents from 2006 to 2015 clustered by location in Brazil", echo=FALSE, out.width="100%"}
...
```{r equip, fig.cap="The shares of accidents coming from the same type of aircraft", echo=FALSE, out.width="100%"}

include_graphics("1.png")

```

```{r manu, fig.cap="The shares of accidents coming from the same manufacturer", echo=FALSE, out.width="100%"}

include_graphics("2.png")

```

```{r model, fig.cap="The shares of accidents coming from the same aircraft model", echo=FALSE, out.width="100%"}

include_graphics("3.png")

```

```{r engtyp, fig.cap="The shares of accidents coming from the same type of aircraft engine", echo=FALSE, out.width="100%"}

include_graphics("4.png")

```

```{r engamo, fig.cap="The shares of accidents coming from the same amount of engine", echo=FALSE, out.width="100%"}

include_graphics("5.png")

```

```{r regav, fig.cap="The shares of accidents coming from the same registration of aviation ", echo=FALSE, out.width="100%"}

include_graphics("6.png")

```

```{r opphase, fig.cap="The shares of accidents coming from the same operation phase", echo=FALSE, out.width="100%"}

include_graphics("7.png")

```

```{r typop, fig.cap="The shares of accidents coming from the same type of operation", echo=FALSE, out.width="100%"}
...
```{r max, fig.cap="The shares of accidents coming from the same type of operation", echo=FALSE, out.width="100%"}
...
```{r seat, fig.cap="The shares of accidents coming from the same type of operation", echo=FALSE, out.width="100%"}
...
```{r bar, fig.cap ="Distribution of the type of occurrences",echo=FALSE}
#pie(data$type.of.occurrence)
barplot(table(data$type.of.occurrence))
```


In Figure \@ref(fig:bar), there are a many causes of an accident to occur and working with these causes will be difficult. One idea is to divide the causes into different categories. We choose to divide them into causes that occur during takeoff, flight, landing or on the ground. Causes that can occur at any time will be designated as unknown. 


```{r pichart, fig.cap = "Categorisation of the multiple type of occurrences",echo=FALSE, out.width="100%"}
par(mai = c(0.3,0.3,0.3,0.3), oma = c(2,0,2,0))
...
incident <- c("takeoff", "during the flight", "landing", "on the ground", "unknown")
...
As shown in Figure \@ref(fig:pichart), most of the occurrences occur during the flight. Surprisingly, there are a lot of occurrences that happen on the ground and a negligible amount that occurs during takeoff. 

Now we will study the severity of the accident depending on the causes. To do so, we have two columns that we focus on: the classification of the accident (if it was a serious incident or an accident) and the damage level on the aircraft. 
First, let us see how the classification can vary depending on the type of occurrences. An accident is an occurrence where the flight has been stopped, whereas a serious incident is an incident that involved circumstances that indicate a high probability of an accident. 

```{r ,fig.cap = "Proportion of the classifcation depending on the type of occurrences", echo=FALSE, out.width="100%"}
...
pie (takeoffserverity$c.nb_accident..nb_serious., labels = paste(round (label), "%", sep = ""), main = "Takeoff")
...
pie (c(1,0), density = NULL)
legend("right", legend = c("Accident", "Serious Incident"), fill = c("white", "cadetblue3"),title = "Classification", cex = 1.3)
```

Most of the occurrences that happened are accidents. For the occurrences that happened during landing, there are 53% serious incident. These results make sense since the occurrences that happens during the landing make the flight stop anyway, because it already arrived to its destination. To summarize we are going to plot the proportions of occurrences for each classification. 

```{r ,fig.cap = "Proportion of types of occurences for each classifcation ", echo=FALSE, out.width="100%"}

par(mfrow=c(1,2), mai = c(0,0.7,0.5,0.7))
...
proportions <- data.frame(incident, dimension)
pie(proportions$dimension, labels = incident, cex = 0.9, main = "Accident")
...
incident <- c("takeoff", "during flight", "landing", "on the ground", "unknown")
...
proportions <- data.frame(incident, dimension)
pie(proportions$dimension, labels = incident, cex = 0.9, main = "Serious Incident")
...
```{r pichart2,fig.cap = "Pie charts of the damage level depending on the classification", echo=FALSE, out.width="100%"}
par(mfrow=c(1,2), mai = c(0,0.7,0.5,0.7))
...
As shown in Figure \@ref(fig:pichart2), incidents have more light or none damage and no destructive damage, where light and no damage are a small part of  the damages that occur during an accident.Also, accident's damages are more severe (approximately 25% of aircrafts are destroyed during an accident compared to 0% during serious incidents). Of course, light damages that occurred during accidents represent around 10%: that may be explained by accidents occurring on the ground and causing less damages. We now plot the proportion of damages for each category. 
 
```{r ,fig.cap= "Pie charts of the damage level depending on the type of occurences", echo=FALSE, out.width="100%"}
par(mfrow=c(2,3), mai = c(0.3,0.3,0.3,0.3))
...
pie (takeoffdamage$c.nb_destroyed..nb_substantial..nb_light..nb_none..nb_unknown., 
     labels = paste(round (label), "%", sep = ""),col = rainbow(5), main = "Takeoff")
...
pie (duringdamage$c.nb_destroyed..nb_substantial..nb_light..nb_none..nb_unknown., 
     labels = paste(round (label), "%", sep = ""),col = rainbow(5), main = "During the flight")
...
As shown in Figure \@ref(fig:linear), most of variables has linearity, except that `registration_category_PRI` and `registration_phase_RUN_AFTER_LANDING` seem to be poor at linearity and might require further data transformation.
...
stargazer(test, type = "html",flip=TRUE, title = "Table 5: VIF value for each variable", digits = 2)
```
...
```{r cook, fig.cap = "Check no influential observations assumption for logistic regression", echo = FALSE}
library(broom)
plot(mylogit, which = 4, id.n = 3)
```

In the Cook's distance in Figure \@ref(fig:cook), outliers in the dataset are presented and the 3 largest distance values are labelled which require us to further explore them.


```{r extreme1, echo = FALSE}
...
```{r extreme2, fig.cap = "Plot out the standard residuals", echo = FALSE}
ggplot(model.data, aes(index, .std.resid)) + 
  geom_point(aes(color = classification), alpha = .5) +
  theme_bw()
```

To filter outliners, we try searching for the points with absolute value of standard residual to be greater than 3. As displayed in Figure \@ref(fig:extreme2), no such points in our dataset hence there is no influential observations.
...
The result of PCA in Figure \@ref(fig:pca) shows that when we keep 7 dimensions, only around 40% of variance will be captured and the curse of dimensionality might still exist. Hence it is not sensible to continue the investigation by performing clustering. Also, the DBSCAN does not give a good result on it hence not included.
